...
### 快照 0060：简历库跨页浏览预加载
- 修改时间：2026-06-30 17:47:56 +08:00
- 修改原因：
  - 用户在第一页第十个候选人点击“下一份”时，需要等待下一页列表和候选人详情加载，体感响应较慢。
  - 希望当前页浏览时提前加载后两页，例如第一页预加载第二、三页，第二页预加载第三、四页。
- 修改文件：
  - `frontend/app.js`
  - `frontend/index.html`
  - `tests/domain/test_frontend_resume_member_view.py`
  - `docs/change-snapshots-2.md`
- 修改结果：
  - 新增 `resumePageCache`，当前简历页加载完成后静默预加载后两页列表。
  - 跨页分页、右键和“下一份”优先使用缓存页，减少第十个候选人跳到下一页时的空等。
